...
Append observations to \`${progressRelPath}\` as you go (one short section per AC or intent point you tested). Use the format:
...
- \`e2e-failed\` — at least one AC / intent point did not work on the preview. Add detailed reproduction steps to \`${failuresRelPath}\`.
- \`e2e-blocked\` — you could not verify (preview unreachable, auth wall, mock user broken). Add a note to \`${failuresRelPath}\` so the orchestrator can decide whether to retry.
...
The orchestrator will re-trigger Planning with your failures.md as context, then re-Building, then a fresh E2E. Be specific in failures.md — vague notes like "tooltip doesn't show" don't help the next Planning Agent decide what to change. Include the URL you tested, the element selector you looked at, the browser state, and what you expected vs. what you saw.
${priorFailuresBlock}
# Hard constraints
...
- Stay focused on the original issue — do not flag unrelated bugs in this verdict (write them to \`${failuresRelPath}\` for the human reviewer if you spot them, but they do not affect the pass/fail).
`;
}
